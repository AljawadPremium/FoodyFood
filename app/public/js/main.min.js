            ...
            (Riode.initProductSingle = (function () {
                ...
                return (
                    ...
                    (e.prototype.variationMatch = function () {
                        ...
                        e.$priceWrap.find("span").text(), e.$priceWrap.slideDown(), e.$clean.slideDown(), e.$btnCart.removeAttr("disabled");
                    }),
            ...
            (Riode.initProductSinglePage = (function () {
                ...
                function o(e) {
                    ...
                    t.hasClass("added") ||
                        ...
                        setTimeout(function () {
                            t.removeClass("load-more-overlay loading").html('<i class="d-icon-heart-full"></i> Browse wishlist').addClass("added").attr("title", "Browse wishlist").attr("href", "wishlist");
                        }, 500));*/
                }
                function a(e) {
                    e.preventDefault();
            ...
            (Riode.Shop = {
                init: function () {
                    this.initProductsQuickview(),
                        ...
                        this.initVariation(),
                        this.initWishlistButton(".btn-wishlist"),
                        this.initWishlistButton(".modal_wishlist"),
                ...
                initProductsQuickview: function () {
                    // page-wrapper class add karne body nanter chya first div la 
                    Riode.$body.on("click", ".btn_quickview", function (e) {
                        var pid = $(this).data("id") // will return the number 123
                        e.preventDefault(),
                            $(this).closest(".dark-theme").length > 0
                                ? Riode.popup(
                                      {
                                          items: { src: "./ajax/quickview/"+pid },
                                          callbacks: {
                                              ajaxContentAdded: function () {
                    ...
                    Riode.$body
                        ...
                        .on("click", ".product:not(.product-variable) .btn-product-icon.btn-cart, .product:not(.product-variable) .btn-product.btn-cart", function (e) {
                            ...
                            o.length > 0
                                ...
                                : n.hasClass("product-single") ||
                                  Riode.Minipopup.open({
                                      // message: "Successfully Added asd asd",
                                      // productClass: "product-cart",
...
$(document).on('click',"a.size.active",function()
{
    var dataid = $(this).data('id');
    var pid = $(this).data('pid');
    $.ajax({
            type: 'POST',
            url: "./product/get_added_count",
            data: {'pid':pid,'metadata':dataid},
            success: function(response)
            {
                $('#loading').hide();
                var response = $.parseJSON(response);
                if (response.status == true)
                {
                    $('.p_qty_'+pid).val(response.data.added_count);
                    // alert(response.data.added_count);
                }
                else
                {
                    // alert(response.message);
...
$(document).on('click',".quantity-plus",function()
{
    ...
    var added_count = $(".p_qty_"+pid).val();
    if (added_count == '0') {
        var increment_by = minimum_add_to_cart;
    }

    // var increment_by = '1';
    var total = parseInt(added_count)+parseInt(increment_by);


    $(".p_qty_"+pid).val(total);
});
...
$(document).on('click',".quantity-minus",function()
{
    ...
    /*check if added qty is in negative*/
    if (added_count < 0) {
        $(".p_qty_"+pid).val("0");
        return false;
    }
...
{
    var pid = $(this).data('id');
    var issizeweight = $(this).data('issizeweight');
    var qty_g = $(".p_qty_"+pid).val();

    if (qty_g == 0){
        alert("Please add quantity."); return false;
        // $(".p_qty_"+pid).val('1');
    }

    if (typeof(qty_g) == "undefined") {
        qty_g = 1;
    }
    
    // qty_g = 1;
    var size = ".sizelist_"+pid;

    var size_value = $(size+" a.size.active").data("id");
    // var asd =   '<div class="input-group mr-2"><button class="quantity-minus d-icon-minus"></button><input class="quantity form-control" type="number" value="12" min="1" max="1000000"><button class="quantity-plus d-icon-plus"></button></div>';

    add_to_cart(pid,qty_g,size_value,issizeweight);
});


function add_to_cart(pid,qty,size_value_g,issizeweight)
{
    var selected = [];
    ...
    var extra = selected.join(',');
    // Display or process selected values
    ...
    $('#loading').show();    
    var metadata = {};
    if (size_value_g === null ){ } else {
        if(typeof issizeweight !== "undefined")
        {
    ...
    if(pid!=='' && qty!=='')
    {
        $.ajax({
            type: 'POST',
            url: "./add-to-cart",
            data: {'pid':pid,'qty':qty,'metadata':metadata,'extra_id':extra},
            success: function(response)
            {
                $('#loading').hide();
                var response = $.parseJSON(response);
                if (response.status == true)
                {
                    var product_name = response.a_data.product_name;
                    var product_price = response.a_data.product_price;
                    var product_image = response.a_data.product_image;
                    var product_detail_link = response.a_data.product_detail_link;
                    var product_count = response.a_data.get_product_count;
                    var checkout_url = response.a_data.checkout_url;
                    var cart_url = response.a_data.cart_url;

                    $('.p_qty_'+pid).val(product_count);
                    $('.product_'+pid+' span').html("Added");

                    var offer = response.product_data.offer;
                    if (offer) 
                    {
                        var all_users = '';
                        $.each(offer, function( index, value ) 
                        {
                            all_users += '<div class="singl_ofr_list '+value['status']+'"  >';
                        ...
                        $('.offer_label_'+pid).html(all_users);
                    }

                    if (response.product_data.price_select != '2') {
                        $('.product_price_'+pid+'').html('₹ '+response.product_data.sale_price);
                    }
                    // $('.qty_price_'+size_class).html(''+response.product_data.qty_label);
                    // $('.price_label_'+size_class).html('₹'+response.product_data.price_label);
                    ...
                    Riode.Minipopup.open({
                        ...
                        productClass: "product-cart",
                        name: product_name,
                        nameLink: product_detail_link,
                        imageSrc: product_image,
                        imageLink: product_detail_link,
                        price: product_price,
                        count: product_count,
                        actionTemplate:'<div class="action-group d-flex"><a href="'+cart_url+'" class="view_Cart_modal_btn">View Cart</a><a href="'+checkout_url+'" class="chk_btn">Check Out</a></div>',
                    });
                    
                    $('.boskery-btn__hover').html(''); 
    ...
    $.ajax({
        ...
        success: function(response)
        {
            if(response.status==true)
            {
                ...
                $(".header_cart_total").text(response.cart_total);
            }
        }
    });
}
...
$(document).on('click',".cart-dropdowna",function()
{
    cart_dropdown();
});
...
function cart_dropdown() {
    $.ajax({
        type: 'POST',
        url: "./view-cart-count",
        data: {'type':"cart_dropdown"},
        dataType:"json",  
        success: function(response)
        {
            if(response.status==true)
            {
                $(".mini_cart").html(response.data.html_tag);
                // $(".cart-dropdown .cart-total").remove();
                // $(".cart-dropdown .cart-action").remove();
                $(".mini_cart").after(response.data.html_tag2);
            }
        }
    });
}

$(document).on("click",".delete_pro_menu",function()
{
    var pid = $(this).data("id");
    removeee_me(pid);
});

function removeee_me(pid)
{
    ...
    if(pid!=''){
        $.ajax({
            ...
            success: function(response)
            {
                ...
                if (response.status == true)
                {
                    /*header view cart demo*/
                    $("."+pid).remove();
                    jQuery('.cart-total .price').text(response.sub_total);
                    var r_count = $('.menu_count_div').length;
